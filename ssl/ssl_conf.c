...
/* Flag is for certificate verification */
...
static int cmd_FIPS(SSL_CONF_CTX *cctx, const char *value)
{
    int rv = 1;
    if( value == NULL || *value == '\0' ||
        ...
        if( ! EVP_default_properties_is_fips_enabled(NULL) ) {
            rv = EVP_default_properties_enable_fips(NULL, 1);
        }
    } else if( strcasecmp( "off", value ) == 0 ||
               strcasecmp( "no", value ) == 0 ||
               strcasecmp( "0", value ) == 0 ) {
        if( EVP_default_properties_is_fips_enabled(NULL) ) {
            rv = EVP_default_properties_enable_fips(NULL, 0);
        }
    } else {
        return -3;
    }
...
static int cmd_CertValidation(SSL_CONF_CTX *cctx, const char *value)
{
    static const ssl_flag_tbl ssl_certval_list[] = {
        ...
        SSL_FLAG_CERTVAL("TrustedFirst", X509_V_FLAG_TRUSTED_FIRST),
        ...
        SSL_FLAG_CERTVAL("NoAltChains", X509_V_FLAG_NO_ALT_CHAINS),
                    ...
                    const char *CAfile, const char *CApath)
{
    X509_STORE **st;
    if (cctx->ctx)
        st = &cctx->ctx->cert_store;
    else if (cctx->ssl && cctx->ssl->ctx)
        st = &cctx->ssl->ctx->cert_store;
    else
        return 1;
    if (*st == NULL) {
        *st = X509_STORE_new();
...
static const ssl_conf_cmd_tbl ssl_conf_cmds[] = {
    ...
    SSL_CONF_CMD_STRING(FIPS, "fips", 0),
